...
public abstract class AbstractAccount implements Account {
	...
	public void remove(Item item) {
		if (item == null) {
			...
			throw new IllegalArgumentException(
					"Can not remove item as it is not in the card");
		} else {
			transactions.remove(new Transaction(item,Account.ADD));
		}
	}
	...
	public void remove(Item item, String dateString) {
		if (item == null) {
			...
			transactions.remove(new Transaction(item,Account.ADD, dateString, dFormat));
		}
	}
